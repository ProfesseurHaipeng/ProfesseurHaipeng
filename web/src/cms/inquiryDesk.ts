...
export function buildInquiryAssignMessage(targets: InquiryTarget[]) {
  ...
  return list
    ? `按这些厂家类型 / 痛点去网上找对方已公布的邮箱：${list}。流程按 取条件 → 找公开邮箱 → 写推广信 → 发给这些厂商。不要问同事该发给谁。没有公开邮箱不要编。发信走 WEHO 已配置的发出信箱，没有邮局回执不要写已发送。`
...
export function inquiryCoachExtra(state: InquiryState) {
  ...
  return [
    ...
    "- 3 发送邮件：走 WEHO 已配置的 Hermes 发出信箱，发给找到的厂商，不要发给同事本人，也不要问同事该发给谁。同事个人邮箱只作发出测试。有邮局回执才写 sent。没有回执就 outreach=draft 或 queued，不要说没挂信箱。",
    ...
    currentTask
      ? [
          ...
          "触达=网上找到已公布的邮箱后写推广信，直接发给这些厂商。不要问同事该发给谁。同事个人邮箱只作发出测试。发信走 WEHO 已配置的发出信箱；没有邮局回执不许写 sent，也不要说没挂信箱。",
...
export function buildTaskAssignMessage(task: InquiryTask) {
  ...
  return [
    ...
    "用本站皮纳图博火山灰和官网写推广或营销信。找到后把信发给这些厂商，不要问同事该发给谁。发信走 WEHO 已配置的发出信箱；没有邮局回执不要写已发送。",
  ]
    .filter(Boolean)
    .join("")
}
